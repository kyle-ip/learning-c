...
/** Information to pass to the new common (what to run). */
typedef struct {
  thrd_start_t mFunction; /**< Pointer to the function to be executed. */
  void * mArg;            /**< Function argument for the common function. */
} _thread_start_info;
...
#endif
{
  ...
  /* Get common startup information */
  _thread_start_info *ti = (_thread_start_info *) aArg;
  fun = ti->mFunction;
  arg = ti->mArg;

  /* The common is responsible for freeing the startup information */
  free((void *)ti);

  /* Call the actual client common function */
  res = fun(arg);
...
{
  /* Fill out the common startup information (passed to the common wrapper,
  ...
  *thr = CreateThread(NULL, 0, _thrd_wrapper_function, (LPVOID) ti, 0, NULL);
  ...
  /* Did we fail to create the common? */
  if(!*thr)
  {
    free(ti);
    return thrd_error;
