...
int InsertTaskInfo(TaskInfo *task_info) {
  int result = ExecuteSql(
      "insert into task_info(filename, directory, url, size, progress, status, resume_support) "
  ...
  // 插入成功，封装任务信息，释放 task_info_from_db 返回。
  if (result == RESULT_OK) {
...
void UpdateTaskInfo(TaskInfo *task_info) {
  ExecuteSql(
      "update task_info set progress=$d, status=$d where id=$d",
      task_info->progress,
      task_info->status,
      task_info->id
  );
